...
async function main() {
    ...
    app.post('/api/webhooks/tendiocentral', express.raw({ type: 'application/json' }), auth.verifyWebhook(), async (req, res) => {
        const { event, event_id, data } = req.webhookEvent;
        switch (event) {
            case 'user.deactivated':
                await db.users.update({
                    where: { tendiocentral_id: data.user_id },
                    data: { is_active: false },
                });
                break;
            case 'session.revoked_all':
                await destroyUserSessions(data.user_id);
                break;
...
//# sourceMappingURL=appendix-b.test.js.map