...
import type { TendioTokenSet } from '../types.js';
...
function makeTokens(expiresInSeconds: number): TendioTokenSet {
  return {
...
describe('refresh mutex', () => {
  it('concurrent calls with same refresh token return the same promise', async () => {
    ...
    const mockTokenSet: TendioTokenSet = {
      access_token: 'new_at',
    ...
    (auth as any).appConfig = mockAppConfig;
    ...
    resolveFetch(
      new Response(JSON.stringify(mockTokenSet), {
        status: 200,
