...
describe('verifyWebhookPayload', () => {
  ...
  it('rejects an invalid signature', () => {
    ...
    ).toThrow(TendioAuthError);

    try {
      verifyWebhookPayload(body, { signature: 'sha256=bad', timestamp }, TEST_SECRET);
    } catch (err) {
      expect((err as TendioAuthError).code).toBe('webhook_signature_invalid');
    }
  });
  ...
  it('rejects a timestamp older than 5 minutes', () => {
    ...
    try {
      ...
      expect((err as TendioAuthError).code).toBe('webhook_timestamp_invalid');
    }
  });
  ...
  it('rejects missing signature header', () => {
    ...
    try {
      ...
      expect((err as TendioAuthError).code).toBe('webhook_payload_malformed');
    }
  });
