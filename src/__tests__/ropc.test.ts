...
async function makeSignedIdToken() {
  ...
  const publicJwk = await jose.exportJWK(
    (await jose.generateKeyPair('RS256')).publicKey,
  );

  const keyPair = await jose.generateKeyPair('RS256');
  const pubJwk = await jose.exportJWK(keyPair.publicKey);
  (pubJwk as any).kid = 'test-key-1';
...
function setAppConfig(auth: TendioAuth) {
  (auth as any).appConfig = {
    appId: 'app1',
    appName: 'Test',
...
describe('loginWithCredentials', () => {
  ...
  it('throws credentials_login_disabled when allowCredentialsLogin is false', async () => {
    ...
    await expect(
      auth.loginWithCredentials('test@example.com', 'pass', 'acme'),
    ).rejects.toThrow(TendioAuthError);

    try {
      await auth.loginWithCredentials('test@example.com', 'pass', 'acme');
    } catch (err) {
      expect((err as TendioAuthError).code).toBe('credentials_login_disabled');
    }
  });
  ...
  it('throws TendioAuthError on 400 invalid_grant', async () => {
    ...
    await expect(
      auth.loginWithCredentials('bad@example.com', 'wrong', 'acme'),
    ).rejects.toThrow(TendioAuthError);

    try {
      await auth.loginWithCredentials('bad@example.com', 'wrong', 'acme');
    } catch (err) {
      expect((err as TendioAuthError).code).toBe('token_exchange_failed');
      expect((err as TendioAuthError).statusCode).toBe(400);
    }
